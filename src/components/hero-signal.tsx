...
export function HeroSignal({ audioSrc }: { audioSrc?: string }) {
  ...
  const [bars, setBars] = useState(() =>
    Array.from({ length: 28 }, (_, index) => 18 + ((index * 19) % 52)),
  );
  ...
  function stopVisualizer() {
    ...
    setBars(Array.from({ length: 28 }, (_, index) => 18 + ((index * 19) % 52)));
  }
  ...
  function startVisualizer() {
    ...
    const tick = () => {
      ...
      const sliceSize = Math.max(1, Math.floor(data.length / 28));
  ...
  return (
    <div className="mt-8 max-w-xl border border-ash-line bg-background/78 p-4 backdrop-blur">
      ...
      <div className="mt-3 flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm leading-6 text-bone">
            {audioSrc
              ? "This is what the dark sounds like. Press play."
              : "The 60-second narrated sample is being mastered."}
          </p>
